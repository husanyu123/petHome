...
@property (nonatomic, strong) JHLikeButton *likeBtn;
...
- (JHLikeButton *)likeBtn{
    if (!_likeBtn) {
        _likeBtn = [[JHLikeButton alloc]init];
//        [_likeBtn setImage:[UIImage imageNamed:@"小心心已点赞？"] forState:UIControlStateNormal];
        _likeBtn.type = JHLikeButtonType_Heart;
        [_likeBtn prepare];
        _likeBtn.clickBlock=^(BOOL like){
            if (like) {
...
- (UIButton *)commentBtn {
    if (!_commentBtn) {
        ...
        _commentBtn.tag=100;
        [_commentBtn setImage:[UIImage imageNamed:@"右Comment"] forState:UIControlStateNormal];
