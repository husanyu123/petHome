...
@interface CJSegmentVC ()<TGSegmentBarDelegate,UIScrollViewDelegate>
@property (nonatomic, weak) UIScrollView *contentV;

@end

@implementation CJSegmentVC
{
...
- (void)viewWillLayoutSubviews {
    ...
    if (self.segmentBar.superview == self.view) {
        self.segmentBar.frame = CGRectMake(0, (IPHONEX_SET?44:20), self.view.width, 35);//如果父视图是自己，那么不要全屏，需要减去segmentBar的高度及64
        CGFloat contentViewY = self.segmentBar.y + self.segmentBar.height;
        //NSLog(@"没有在navigationItem.titleView->%f",self.view.height);
        CGRect contentFrame = CGRectMake(0, 0, self.view.width, self.view.height - contentViewY);
